...
_optional(false)
{
    ...
    _getByNameCache.set_function(std::bind(&HlaDynamicalEntityManagerImpl::findByName, this, std::placeholders::_1));
}
...
HlaDynamicalEntityImplPtr HlaDynamicalEntityManagerImpl::findByName(std::wstring name) {
    ...
    return HlaDynamicalEntityImplPtr();
}

HlaDynamicalEntityPtr HlaDynamicalEntityManagerImpl::getDynamicalEntityByName(std::wstring name) {
    {
        std::lock_guard<std::mutex> lock(_instancesLock);
        // implicit call to findByName if not found in cache
        HlaDynamicalEntityImplPtr cachedInstance = _getByNameCache(name, _localInstances.size() + _remoteInstances.size());
    ...
    return getDynamicalEntityByName(name);
}
...
THROW_SPEC (HlaNotConnectedException, HlaInternalException, HlaRtiException, HlaInvalidLogicalTimeException, HlaSaveInProgressException, HlaRestoreInProgressException) {
...

    {
        ...
        if (dynamicalEntityImpl->hasName()) {
            _getByNameCache.remove(dynamicalEntityImpl->getName());
        }
    }
