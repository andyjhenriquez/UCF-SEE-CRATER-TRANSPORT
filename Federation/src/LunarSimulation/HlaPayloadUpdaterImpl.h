 ...
 */
#ifndef DEVELOPER_STUDIO_HLAPAYLOADUPDATERIMPL_H
#define DEVELOPER_STUDIO_HLAPAYLOADUPDATERIMPL_H
...
namespace LunarSimulation {
    ...
    class HlaPayloadUpdaterImpl : public HlaPayloadUpdater , public std::enable_shared_from_this<HlaPayloadUpdaterImpl> {
    ...
    private:
        ...
        HlaPayloadImplPtr _instance;
        bool _updateSent;

    public:
        explicit HlaPayloadUpdaterImpl(HlaPayloadImplPtr instance) :
            _instance(instance),
            _updateSent(false)
        {}

        void setPosition(const std::vector</* 3 */ double >& position);
        void setVelocity(const std::vector</* 3 */ double >& velocity);
